...
namespace ts
{
    Packet read(char* bytes)
    {
        Packet packet
        {
            ...
            static_cast<uint>(((bytes[1] << 8) | bytes[2]) & 0x1FFF),   // Packet Identifier
            static_cast<uint>((bytes[3] & 0xC0) >> 6),                  // Transport Scrambling Control
            static_cast<uint>((bytes[3] & 0x30) >> 4),                  // Adaptation Field Control
            static_cast<uint>((bytes[3] & 0x0F)),                       // Continuity Counter
            ""
        };
